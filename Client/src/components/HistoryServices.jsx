...
const HistoryServices = ({ history }) => {
  ...
  return (
    <section className='w-full mx-auto my-10 flex flex-col flex-wrap justify-evenly gap-10 md:flex-row'>
        ...
        {history.length >= 1 ? <h2 className='font-medium text-center text-xl dark:text-darkText'> Procedimientos anteriores </h2> : <h2 className='font-medium text-center'> Sin procedimientos registrados </h2>}
      {history.map((service, index) => {
        return (
          <div key={index} className='flex flex-row flex-wrap justify-between shadow-sm shadow-black p-2'>
            ...
            <div className='flex flex-col justify-end pb-2 gap-2'>
              <p className="cursor-pointer shadow-md rounded-2xl w-40 px-5 bg-beige hover:bg-blue-600 hover:text-white dark:bg-darkPrimary dark:text-darkText dark:hover:bg-blue-600 ">           

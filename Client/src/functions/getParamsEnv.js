...
export default function getParamsEnv() {
  ...
  const CLIENTDETAILBASE =
    import.meta.env.VITE_CLIENT_DETAIL_BASE || "/client-detail";
  const CLIENTDETAIL =
    import.meta.env.VITE_CLIENT_DETAIL || "/client-detail/:id";
  const CLIENTSPROFILES =
    import.meta.env.VITE_CLIENTS_PROFILES || "/clientsProfiles";
  const TERMSANDPRIVACY =
    import.meta.env.VITE_TERMS_AND_PRIVACY || "/terms-and-privacy";
  const API_URL_BASE =
  ...
  const HISTORYPRICE =
    import.meta.env.VITE_HISTORY_PRICE || "/historyprice/:productId";
  const HISTORYPRICEBASE =
    import.meta.env.VITE_HISTORY_PRICE_BASE || "/historyprice";
